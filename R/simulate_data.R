...
sim_count <- function(seed = NULL, n_genes = 1000, n_cell_types = 3, cell_type_prob = rep(1, n_cell_types),
  ...
  #   }
  # }
  # Always draw the randomness, even if not used
  mu_per_group <- rnorm(n_groups, mean = 0, sd = 1)
  var_per_group <- rinvgamma(n_groups, shape = 3, scale = 2)
  cell_cont_values <- rnorm(
    nrow(metadata),
    mean = if (is_cc_confounder) mu_per_group[match(metadata$group, groups)] else 0,
    sd   = if (is_cc_confounder) sqrt(var_per_group[match(metadata$group, groups)]) else 1
  )

  # Conditionally assign
  if (include_cell_cont) {
    metadata[, cell_continuous := cell_cont_values]
  }
  ...
  # Always define the probabilities
  probs_list <- lapply(seq_len(n_groups), function(i) {
    p <- runif(3)
    p / sum(p)
  })
  default_probs <- c(0.45, 0.35, 0.2)

  # Always simulate the values
  cell_disc_values <- if (is_cd_confounder) {
    mapply(function(grp) {
      sample(levels, 1, prob = probs_list[[match(grp, groups)]])
    }, metadata$group)
  } else {
    sample(levels, nrow(metadata), replace = TRUE, prob = default_probs)
  }

  # Conditionally assign
  if (include_cell_disc) {
    metadata[, cell_discrete := cell_disc_values]
  }
  ...
  sample_cont_values <- if (is_sc_confounder) {
    rnorm(
      nrow(sample_covariate),
      mean = mu_per_group[match(sample_covariate$group, groups)],
      sd   = sqrt(var_per_group[match(sample_covariate$group, groups)])
    )
  } else {
    rnorm(nrow(sample_covariate), mean = 0, sd = 1)
  }

  # Conditionally assign and merge
  if (include_sample_cont) {
    sample_covariate[, sample_continuous := sample_cont_values]
    metadata <- merge(metadata, sample_covariate[, .(sample, sample_continuous)], by = "sample")
  }
  ...
  default_probs <- c(0.5, 0.5)

  # Always simulate values
  sample_covariate <- data.table(sample = sample_ids, group = group_labels)

  sample_disc_values <- if (is_sd_confounder) {
    mapply(function(grp) {
      sample(levels, 1, prob = probs_list[[match(grp, groups)]])
    }, sample_covariate$group)
  } else {
    sample(levels, nrow(sample_covariate), replace = TRUE, prob = default_probs)
  }

  # Conditionally assign and merge
  if (include_sample_disc) {
    sample_covariate[, sample_discrete := sample_disc_values]
    metadata <- merge(metadata, sample_covariate[, .(sample, sample_discrete)], by = "sample")
  }
  ...
  total_cell_type_specific <- n_ct_specific * n_cell_types #- similarity * (n_cell_types - 1)

  if (total_cell_type_specific > n_genes) {
    stop("The total number of cell type-specific genes exceeds the total number of available genes. Please provide a different value for 'ct_specific_prop' or 'n_cell_types'.")
  }
  ...
  n_true_specific_group <- n_grp_specific - n_common_group
  ...
  for (i in seq_along(groups)) {
    ...
    signs <- sample(c(-1, 1),
      size = n_grp_specific, replace = TRUE,
  ...
  # cell-level continuous
  # Always generate effects
  idx_cell_cont <- sample(seq_len(n_genes), round(n_genes * cell_cont_prop))
  cell_cont_effects <- numeric(n_genes)
  cell_cont_effects[idx_cell_cont] <- rnorm(length(idx_cell_cont), mean = 0, sd = 0.3)

  if (include_cell_cont) {
    covariate_effects$cell_cont <- cell_cont_effects

    colname <- "cell_level_continuous_covariate_association"
    gene_info[, (colname) := FALSE]
    gene_info[idx_cell_cont, (colname) := TRUE]
  }
  # cell-level discrete
  # Always determine which genes are affected
  idx_cell_disc <- sample(seq_len(n_genes), round(n_genes * cell_disc_prop))

  # Simulate two effects per selected gene: one for B vs A, one for C vs A
  cell_disc_effects_B <- numeric(n_genes)
  cell_disc_effects_C <- numeric(n_genes)

  cell_disc_effects_B[idx_cell_disc] <- rnorm(length(idx_cell_disc), mean = 0, sd = 0.3)
  cell_disc_effects_C[idx_cell_disc] <- rnorm(length(idx_cell_disc), mean = 0, sd = 0.3)

  # Store in list only if covariate is used
  if (include_cell_disc) {
    covariate_effects$cell_disc_B <- cell_disc_effects_B
    covariate_effects$cell_disc_C <- cell_disc_effects_C

    colname <- "cell_level_discrete_covariate_association"
    gene_info[, (colname) := FALSE]
    gene_info[idx_cell_disc, (colname) := TRUE]
  }
  # sample-level continuous
  # Always generate effects
  idx_sample_cont <- sample(seq_len(n_genes), round(n_genes * sample_cont_prop))
  sample_cont_effects <- numeric(n_genes)
  sample_cont_effects[idx_sample_cont] <- rnorm(length(idx_sample_cont), mean = 0, sd = 0.3)

  if (include_sample_cont) {
    covariate_effects$sample_cont <- sample_cont_effects

    colname <- "sample_level_continuous_covariate_association"
    gene_info[, (colname) := FALSE]
    gene_info[idx_sample_cont, (colname) := TRUE]
  }
  # sample-level discrete
  # Always generate effects
  idx_sample_disc <- sample(seq_len(n_genes), round(n_genes * sample_disc_prop))
  sample_disc_effects_Y <- numeric(n_genes)
  sample_disc_effects_Y[idx_sample_disc] <- rnorm(length(idx_sample_disc), mean = 0, sd = 0.3)

  if (include_sample_disc) {
    covariate_effects$sample_disc_Y <- sample_disc_effects_Y

    colname <- "sample_level_discrete_covariate_association"
    gene_info[, (colname) := FALSE]
    gene_info[idx_sample_disc, (colname) := TRUE]
  }
  ...
  colnames(count_mat) <- metadata$cell_id
  ...
  for (j in seq_len(n_cells)) {
    ...
    count_mat[, j] <- rnbinom(n_genes, mu = mu, size = 1 / phi)
  }
...
#' Simulate Ligand-Receptor Pairs
#'
#' Simulate ligand-receptor pairs from given gene lists, taking into account subunit architecture.
#'
#' @param seed random seed to use.
#' @param genes.l,genes.r vectors specify genes as potential ligands and receptors respectively.
#' @param dimer numeric. Probability of ligand/receptor consisting of two subunits.
#' @param trimer numeric. Probability of ligand/receptor consisting of three subunits.
#' @param n_lr desired number of ligand-receptor pairs.
#'
#' @returns description
#'
#' @examples
#' # Simulate some ligand-receptor pairs using the default
#' lr <- sim_lr(seed = 2)
#' head(lr)
#'
#' # Simulate some binary ligand-receptor pairs
#' lr.bin <- sim_lr(seed = 3, dimer = 0, trimer = 0)
#' head(lr.bin)
#'
#' @export

sim_lr <- function(seed = NULL, genes.l = paste0("G", 1:500), genes.r = paste0("G", 501:1000),
                   dimer = 0.25, trimer = 0.1, n_lr = 100) {
  if (!is.null(seed)) {
    set.seed(seed)
  }
  ...
  make_complex <- function(genes, dimer, trimer) {
    ...
    simulated_lr <- character()
    # Function to create a complex or single gene
    while (length(genes) > length(used_genes)) {
      remaining_genes <- setdiff(genes, used_genes)

      prob <- runif(1)

      if (length(remaining_genes) == 2) {
        prob <- runif(1, min = trimer, max = 1)
      } else if (length(remaining_genes) == 1) {
        used_genes <- union(used_genes, remaining_genes)
        simulated_lr <- append(simulated_lr, remaining_genes)
        break
      }
      ...
      if (prob < trimer) {
        ...
        simulated_lr <- append(simulated_lr, paste(sort(complex_genes), collapse = "_"))
      } else if (prob < dimer + trimer) {
        ...
        simulated_lr <- append(simulated_lr, single_gene)
      }
    }
  ...
  sim_ligands <- make_complex(genes = genes.l, dimer = dimer, trimer = trimer)
  sim_receptors <- make_complex(genes = genes.r, dimer = dimer, trimer = trimer)

  all_combos <- expand.grid(ligand = sim_ligands, receptor = sim_receptors, stringsAsFactors = FALSE)

  if (nrow(all_combos) < n_lr) {
    stop("'n_lr' is too large, or there are too few genes in 'genes.l' or 'genes.r'")
  }

  interaction_df <- all_combos[sample(nrow(all_combos), n_lr), ]

  interaction_df <- interaction_df[order(interaction_df$ligand, interaction_df$receptor), ]
